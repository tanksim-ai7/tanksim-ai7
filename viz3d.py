...
def _snapshot(fm, drive, detect_mod):
    ...
    return {
        ...
        "my_spd": round(mspd, 1),
        # 대시보드용 추가 항목
        #   body_rate  경로팀이 노출하면 그 값, 없으면 None
        #   mv         경로팀 주행 상태 (MOVE / ARRIVED 등)
        "body_rate": (round(float(getattr(drive, "body_rate", 0.0)), 1)
                      if drive is not None
                      and getattr(drive, "body_rate", None) is not None
                      else None),
        "mv": (getattr(drive, "state", None)
               or getattr(drive, "drive_state", None) or "-"),
        "en_spd": round(espd, 1),
        "swapped": SWAP_SIDES,
        "dist": st.get("dist"),
        "fc": st.get("state", "-"),

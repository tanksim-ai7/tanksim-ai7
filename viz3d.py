...
def attach_viz_taek(app, fm=None, drive=None, detect=None):
    ...
    if "viz3d" not in app.blueprints:
        ...
        # 서버가 뜨자마자 .map 파일을 읽어본다. 문제가 있으면 여기서 바로 드러난다.
        try:
            n = len(_load_map_obstacles())
            print("[viz3d] 맵 로딩 완료 -> %s (오브젝트 %d개)" % (_CACHE.get("map_source"), n))
        except Exception as e:
            print("[viz3d] 맵 로딩 실패:", e)
...
# Display map: read directly from this file beside viz3d.py at server startup.
MAP_FILE = "NewMap_final_5.map"
...
def _load_map_data():
    """선택한 .map 파일을 서버 시작 시 직접 읽는다.

    파일이 없거나 형식이 잘못되면 오브젝트 없는 빈 맵으로 대체하고,
    콘솔에 바로 로그를 남긴다. (예전처럼 손으로 붙여넣은 내장 맵으로
    조용히 대체하지 않는다 — 문제가 있으면 바로 드러나야 한다.)
    """
    ...
    if os.path.isfile(path):
        try:
            ...
            print("[viz3d] 맵 파일 읽기 실패, 빈 맵으로 대체:", e)
    else:
        print("[viz3d] 맵 파일 없음:", path)
